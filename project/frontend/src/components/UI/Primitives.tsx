...
type ButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  loading?: boolean;
  /** 'md' is the default app button; 'lg' is the roomier form/CTA size. */
  size?: 'md' | 'lg';
};
...
export function Button({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 ${sizes}
...
/**
 * Roomier input for the public sign-up screens (register + accept invite) —
 * bigger padding, larger text, softer corners — so those forms feel welcoming
 * rather than dense. Kept separate from `inputClass` so the compact inputs used
 * throughout the admin screens are unaffected.
 */
export const inputClassLg =
  'w-full rounded-xl border border-gray-300 px-4 py-4 text-base ' +
